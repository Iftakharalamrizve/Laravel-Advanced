                        ...
                        <div class="col-md-12">
                            <form method="POST" action="{{ route('admin.roles.update',$role->id) }}" autocomplete="off">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <label>Name*</label>
                                        <input id="name" type="name" name="name" class="form-control" placeholder="Enter name" value="{{$role->name}}">
                                        @error('name')
                                ...
                                @forelse($permissions->chunk(3) as $key=>$chunks)
                                    <div class="form-row">
                                        @foreach($chunks as $key=>$module)
                                            <div class="col-lg-4">
                                                <input name="permission[]" class="form-check-input" type="checkbox" id="inlineCheckbox1" value="{{$module->id}}" {{in_array($module->id, $rolePermissions) ? 'checked' : ''}}>
